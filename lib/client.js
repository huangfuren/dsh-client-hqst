    ...
    `;
		...
		function HistoryPanel(props) {
			...
			React.useEffect(() => {
				...
				let alive = true;
				const binding = sessionsService.binding(currentId);
				if (binding === undefined) {
					setSnapshot(null);
					return undefined;
				}
				const face = binding.session;
				const sync = () => { if (alive) setSnapshot(face.getSnapshot()); };
				sync();
				const dispose = face.subscribe(sync);
				return () => { alive = false; dispose(); };
			}, [currentId]);

			if (currentId === undefined) return null;

			const questions = snapshot === null ? [] : extractQuestions(snapshot);
		...
		function apply(ctx) {
			...
			slots.inject("shell.overlay", () => slots.register(
				...
				(props) => React.createElement(HistoryPanel, { useSessions: props.useSessions }),
			));
		}
